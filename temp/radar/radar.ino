...
uint16_t getDistance() {
  ...
  return (uint16_t)((duration * 17UL / 1000)*5);  // microseconds → cm
}
...
void trackObjects() {
  ...
  for (uint8_t i = 0; i < objectCount; i++) {
    ...
    if (currentDist + 5 < prevDist) {   // object approaching
      Serial.println(F("A"));            // alert Python
      tone(buzzerPin, 1000, BEEP_DURATION);
      delay(BEEP_DURATION);
      return;                            // re-sweep immediately
    }
  }
